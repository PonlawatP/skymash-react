...
export default function SkyCardBadge(props:any) {
    ...
    return <div className="flex flex-col items-center gap-2">
        <div className="flex gap-2">
            <span className={`px-2 pt-1 rounded-md drop-shadow-md ${rank==1? "bg-yellow-400" : rank==2?"bg-slate-300" :rank==3?"bg-orange-300":"bg-white/50" }`}>{rank != 9999 ? `#${rank}` : "ยังไม่ถูกโหวต"}</span>
